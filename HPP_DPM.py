...
from Auxiliary_functions import E_log_p_of_z_given_other_z, ELBO_term_for_pz,\
                                          surrogate_lowerbound, lowerbound, Update_MHPP_parameters,\
                                             Update_HPP_parameters
...
def HPP_DPMM(X, T=10, max_iter=100, alpha=1., thresh=1e-3, delta=.1):
    ...
    for batch in range(B):
        ...
        # Power priors for every global variable       
        omega = 0
        E_q_rho = .5
        ...
        for iteration in range(max_iter):
            ...
            if batch > 0:
                ...
                for t in range(T):
                    prior_cov[t] = E_q_rho*static_cov_mu[t] + (1-E_q_rho)*np.identity(M)
                    prior_mean[t] = np.matmul(np.linalg.inv(prior_cov[t]),\
                                              E_q_rho*static_cov_mu[t].dot(static_mean_mu[t]))
                    b_0[t] = E_q_rho*(static_b_tao[t]) + 1. - E_q_rho
                    a_0[t] = E_q_rho*(static_a_tao[t]) + 1. - E_q_rho
           ...
           # Update tao
            ...
            # else:
            #     log_l.append(surrogate_lowerbound(X, T, N, M, alpha, static_mean_mu, static_cov_mu, static_a_tao,\
